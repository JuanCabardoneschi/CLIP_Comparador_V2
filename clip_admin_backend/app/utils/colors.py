...
def normalize_color(color_str: Optional[str], client_id: Optional[str] = None) -> Optional[str]:
    """
    Normaliza nombres de colores usando LLM con un preprocesamiento mínimo
    para corregir errores morfológicos frecuentes del español.
    ...
    Estrategia:
    - Correcciones léxicas mínimas antes del LLM (ej: "grices" → "gris")
    ...
    # Correcciones específicas mínimas (evita falsos positivos semánticos)
    # Caso histórico: "grices" (error común) debe mapear a "gris"
    if s == "grices":
        return "gris"
