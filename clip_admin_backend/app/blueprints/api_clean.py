...
# ?? CACH� GLOBAL DE EMBEDDINGS (evita recalcular en cada request)
_CATEGORY_EMBEDDINGS_CACHE = {}
_COLOR_EMBEDDINGS_CACHE = {}

# spaCy es OBLIGATORIO para tokenizaci�n (no opcional)
_USE_SPACY_NORMALIZER = True  # Siempre activo
_SPACY_NLP = None

def _get_spacy_nlp():
    """Carga perezosa del modelo spaCy espa�ol (OBLIGATORIO)."""
    global _SPACY_NLP
...
def _get_category_embedding(category_name: str, client_id: str):
    """
    Obtiene embedding de categor�a desde BD persistida o lo calcula si no existe.
...
def _clip_prompt_for_category(category) -> str:
    ...
    2. category.name_en (nombre en ingl�s)
    3. category.name (nombre en espa�ol sin traducci�n)
...
def dashboard_stats():
    ...
    stats = {
        ...
        "recent": {
            ...
            # ).count()
            "searches_today": 0  # Deshabilitado - no se registran b�squedas
        },
...
def _validate_visual_search_request():
    ...
    - Éxito: (client, image_file, None, None)
...
# Funci�n movida a app.blueprints.search_visual
...
## Funci�n detect_image_category movida a app.blueprints.search_visual
...
def _filter_diverse_categories(categories_with_scores, diversity_threshold=0.75):
    ...
    try:
        ...
        # Generar embeddings de nombres de categor�as (usando nombre tal cual para no hardcodear)
        ...
        print(f"\nDIVERSITY FILTER: Matriz de similitud entre {len(category_names)} categor�as:")
        ...
        # Preparar guardas NUM�RICAS (agn�sticas al dominio) para no colapsar categor�as fuertes
        # Evita hardcodear palabras. Si dos categor�as est�n FUERTES o muy cercanas en confianza,
        ...
        strong_conf_threshold = 0.75  # categor�as muy seguras (independiente del rubro)
        ...
        def is_strong_pair(i: int, j: int) -> bool:
            ...
            if ci >= strong_conf_threshold and cj >= strong_conf_threshold:
                return True
            # Muy cercanas entre s� y al menos razonables (>=0.70)
            if abs(ci - cj) <= epsilon_conf and max(ci, cj) >= 0.70:
                return True
            return False

        # Clustering greedy: agrupar categor�as similares SOLO si no violan las guardas num�ricas
        groups = []
        used = set()
        ...
        for i in range(len(categories_with_scores)):
            ...
            for j in range(i+1, len(categories_with_scores)):
                ...
                # NO agrupar si es un par fuerte o muy cercano en confianza (multi-label v�lido)
        ...
        print(f"DIVERSITY FILTER: {len(categories_with_scores)} -> {len(filtered)} categor�as")
        return filtered
...
def detect_multiple_categories(image_data, client_id, min_prob_threshold=0.03, min_conf_threshold=0.18, prelimit_topk=8):
    ...
    try:
        ...
        # Similaridad contra centroides de cada categor�a (mismo m�todo que SINGLE)
        confidences = []
        ...
        # Probabilidades normalizadas v�a softmax de similitudes escaladas
        ...
        # FIX: Obtener max_conf de todos los candidatos (m�xima confidence global)
        max_conf = max(c['confidence'] for c in candidates)
        dominant_threshold = 0.80  # Threshold m�s alto para considerar "dominante"
        is_dominant = max_conf > dominant_threshold

        if is_dominant:
            # Modo ESTRICTO: evitar falsos positivos en im�genes de un solo objeto
            print(f"?? MULTI-CATEGORY: Modo ESTRICTO (max confidence={max_conf:.3f})")
            strict_conf = 0.75  # 75% de similitud m�nima (muy alta)
        ...
        print(f"PRELIMIT: {len(selected)} categor�as seleccionadas (de {len(categories)} totales)")
        for c in selected:
            print(f"   - {c['category'].name}: prob={c['probability']:.4f}, conf={c['confidence']:.4f}")

        # Filtro de diversidad: m�s conservador y con salvaguarda
        # - Si hay 2 o menos categor�as, no colapsar: permiten casos claros de 2 prendas (p. ej., top + short)
        # - Umbral muy alto (0.95) para evitar colapsar categor�as con diferencias sutiles pero importantes (p. ej., "tiro alto" vs "tiro bajo")
        if len(selected) <= 2:
            railway_log(" DEBUG: Saltando filtro de diversidad (<=2 categor�as prelimit)")
        else:
            railway_log(f" DEBUG: Aplicando filtro de diversidad (threshold=0.95)...")
            # Filtrar por diversidad sem�ntica (evita duplicados muy cercanos)
