...
@gpt4v_bp.route('/test')
def test_page():
    """Página de prueba para GPT-4 Vision"""
    # Buscar cliente Goody Store para cargar sus categorías
    from app.models.client import Client
    client = Client.query.filter_by(name="Goody Store").first()
    client_id = str(client.id) if client else None
    return render_template('gpt4v_test.html', client_id=client_id)


@gpt4v_bp.route('/detect', methods=['POST'])
def detect_categories():
    """
    Endpoint para detectar múltiples categorías con GPT-4 Vision
    ...
    try:
        ...
        if custom_categories:
            ...
            # Cargar categorías de BD (solo hojas, no categorías padre)
            from app.models.category import Category
            categories = Category.query.filter_by(
                client_id=client_id,
                is_active=True,
                is_leaf=True  # Solo categorías hoja (sin subcategorías)
            ).all()
            ...
            if not categories_list:
                return jsonify({
                    'success': False,
                    'error': f'Cliente {client_id} no tiene categorías activas (hojas)'
                }), 400
        else:
            # Categorías por defecto del usuario
            categories_list = [
        ...
        # Detectar categorías
        result = detect_categories_with_gpt4v(
