...
def visual_search():
    ...
    try:
        ...
        if multi_category_enabled:
            ...
            # Detectar mÃºltiples categorÃ­as (thresholds hardcodeados)
        ...
        print(f"ðŸŽ¯ DEBUG: Productos encontrados en categorÃ­a {detected_category.name}: {len(product_best_match)}")
...
def text_search():
    ...
    try:
        ...
        # Fallback 2: Si tras el scoring no hay resultados, intentar una bÃºsqueda global sin categorÃ­a
        if len(results) == 0 and detected_category is not None:
            print("âš ï¸ TEXT SEARCH: 0 resultados tras filtrar por categorÃ­a â†’ Reintentando global")
            detected_category = None
            # reconstruir query sin filtro de categorÃ­a
            products_query = db.session.query(
                Product.id,
            ...
            print(f"ðŸ” TEXT SEARCH (fallback): Analizando {len(products)} productos...")
            ...
            results.sort(key=lambda x: x['final_score'], reverse=True)
            results = results[:limit]
            print(f"ðŸ” TEXT SEARCH Fallback: {len(results)} resultados")
