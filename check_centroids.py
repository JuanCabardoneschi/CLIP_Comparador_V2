...
if __name__ == '__main__':
    ...
    spec = importlib.util.spec_from_file_location("flask_app", os.path.join(backend_path, "app.py"))
    flask_app = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(flask_app)

    # Now we can use create_app
    from app import db
    ...
    with app.app_context():
        ...
        for client in clients:
            ...
            for cat in categories:
                ...
                img_count = cat.centroid_image_count or 0
                print(f"   {status} {cat.name:<40s} | Productos: {products_with_images:<3d} | Centroide: {has_centroid} ({img_count} imgs)")
